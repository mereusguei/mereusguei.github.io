...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        let tableHtml = `<table><thead><tr><th>Posição</th><th>Usuário</th><th>Pontuação Total</th></tr></thead><tbody>`;
        if (rankingData.length === 0) {
            tableHtml += '<tr><td colspan="3" style="text-align:center;">Nenhuma pontuação registrada ainda.</td></tr>';
        } else {
            rankingData.forEach((player, index) => {
                tableHtml += `<tr><td><b>${index + 1}º</b></td><td>${player.username}</td><td>${player.total_points}</td></tr>`;
            });
        }
